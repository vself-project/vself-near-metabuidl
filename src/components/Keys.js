...
export const Keys = ({ near, update, localKeys }) => {
  ...
  const deleteAccessKeys = async () => {
    update('loading', true);
    // WARNING NO RESTRICTION ON THIS ENDPOINT
    const result = await fetch('http://localhost:3000/delete-access-keys').then((res) =>
      res.json(),
    );
    if (result && result.success) {
  ...
  return (
    <>
      ...
      {localKeys && localKeys.seedPhrase ? (
        <>
          <p>
            <b>Seed Phrase:</b> {localKeys.seedPhrase}
          </p>
          <p>
            <b>Implicit Account Id:</b> {localKeys.accountId}
          </p>
          <p>
            <b>App Key:</b> {localKeys.accessPublic}
          </p>
          <button onClick={() => getNewAccessKey(true)}>Get New App Key</button>
          <br />
          <button onClick={() => deleteAccessKeys()}>Remove Account</button>(warning removes all
          access keys from contract, for you and everyone else)
        </>
      ) : (
        <>
          <p>
            Creates a seed phrase + access key to interact with the app. Normally you would set up
            your seed phrase with a wallet and the app would add an access key.
          </p>
